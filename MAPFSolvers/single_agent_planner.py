...
def is_constrained(curr_loc, next_loc, next_time, constraint_table):
    ...
    if constraint_table is not None and str(next_time) in constraint_table:
        ...
        for next_time_vertex_constraint_loc in next_time_vertex_constraints_loc:
            ...
            if curr_loc is not None and curr_loc == next_time_vertex_constraint_loc and str(next_time-1) in constraint_table:
...
def a_star(my_map, start_loc, goal_loc, h_values, agent, constraints):
    ...
    while len(open_list) > 0:
            ...
            # FIXME : Why the max here? 
                ...
                if currNode['timestep'] <= max_timestep_constraint:
                    for ts_onwards in range(currNode['timestep'], max_timestep_constraint + 1):                    
                            ...
                            if str(ts_onwards) in constraint_lookupTable[str(agent)] and goal_loc in constraint_lookupTable[str(agent)][str(ts_onwards)]['loc'] :
        ...
        for dir in range(5):            
            ...
            childNode_loc = move(currNode['loc'], dir)
            ...
            if is_constrained(currNode['loc'], childNode_loc, currNode['timestep'] + 1, constraint_lookupTable[str(agent)] if str(agent) in constraint_lookupTable else None):
                continue
